...
public static class JapaneseNumber
{
    ...
    public static IReadOnlyList<string> ToVariants(int n)
    {
        ...
        var roman = ToRoman(n);
        if (roman != null) list.Add(roman);
        var kanji = ToKanji(n);
        if (kanji != null) list.Add(kanji);
        // 丸囲み（1-50だけ標準フォントにある）
        if (n is >= 1 and <= 50)
    ...
    public static string? ToKanji(int n)
    {
        if (n == 0) return "〇";
        if (n < 0) return null;
        if (n >= 1_0000_0000_0000L) return null; // 兆オーバー
        ...
        if (oku > 0) { sb.Append(KanjiBelow10000((int)oku)); sb.Append("億"); }
        if (manSection > 0) { sb.Append(KanjiBelow10000((int)manSection)); sb.Append("万"); }
        if (lower > 0) sb.Append(KanjiBelow10000((int)lower));
        return sb.ToString();
        ...
        static string KanjiBelow10000(int x)
        {
            ...
            var q10 = (x / 10) % 10;
            var q1 = x % 10;
            if (q1000 > 0) { sb2.Append(q1000 == 1 ? "" : d[q1000]); sb2.Append("千"); }
            if (q100 > 0) { sb2.Append(q100 == 1 ? "" : d[q100]); sb2.Append("百"); }
            if (q10 > 0) { sb2.Append(q10 == 1 ? "" : d[q10]); sb2.Append("十"); }
            if (q1 > 0) sb2.Append(d[q1]);
            return sb2.ToString();
        }
    }
