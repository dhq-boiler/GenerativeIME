...
public sealed class JapaneseReader : IDisposable
{
    ...
    {
        if (string.IsNullOrEmpty(candidateText)) return false;
        if (string.IsNullOrEmpty(targetHiragana)) return false;

        var actual = ToHiragana(candidateText);
        if (actual == targetHiragana) return true;
    ...
    private static void AppendKatakanaAsHiragana(StringBuilder sb, string kata)
    {
        foreach (var ch in kata)
        {
            ...
            else if (ch == 'ヵ') sb.Append('ゕ');
            else if (ch == 'ヶ') sb.Append('ゖ');
            else sb.Append(ch);
        }
    }
