...
HRESULT SkkDictionary::Load(const std::wstring& path)
{
    ...
    // Emoji dictionary (generated by scripts/mine/fetch_emoji_dict.ps1)
    // rides along from the same directory when present. Parsed AFTER the
    // main dict so emoji merge onto the TAIL of shared readings (いぬ →
    // 犬 … 🐶), and its kana keys register as direct okuri-nashi entries —
    // which is exactly right: emoji can never pass the MeCab ReadsAs
    // filter, so they need the same dict-maintainer-explicit bypass that
    // 「こんにちわ /今日は/」 gets. Optional: absence is not an error.
    {
        std::wstring emojiPath = path;
        size_t slash = emojiPath.find_last_of(L"\\/");
        if (slash != std::wstring::npos)
        {
            emojiPath.resize(slash + 1);
            emojiPath += L"SKK-JISYO.emoji.utf8";
            ParseFile(emojiPath, deferredOkuri);
        }
    }
